  ...
  moduleServer(id, function(input, output, session) {
    ...
    .filtered_panel <- function(which = c("A","B")) {
      ...
      if (exists("user_email") && is.function(user_email) &&
        ...
        visible_pitchers <- unique(df$Pitcher)
        if (!is.null(player) && !identical(player, "All") && !(player %in% visible_pitchers)) {
...
server <- function(input, output, session) {
  ...
  admin_emails <- c("jgaynor@pitchingcoachu.com", "nathancole@fas.harvard.edu", "jeffrey_kane@fas.harvard.edu", "mslattery@fas.harvard.edu", "cleaden24@gmail.com")
